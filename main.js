...
global.authFile = `SharksBotSession`
...
mobile: MethodMobile, 
browser: opcion == '1' ? ['SharksBot-MD', 'Edge', '2.0.0'] : methodCodeQR ? ['SharksBot-MD', 'Edge', '2.0.0'] : ['Ubuntu', 'Chrome', '110.0.1587.56'],
auth: {
creds: state.creds,
...
if (opts['autocleartmp'] && (global.support || {}).find) (tmp = [os.tmpdir(), 'tmp', "SharksBotMd"], tmp.forEach(filename => cp.spawn('find', [filename, '-amin', '2', '-type', 'f', '-delete'])))}, 30 * 1000)}
...
if (reason == 405) {
await fs.unlinkSync("./SharksBotSession/" + "creds.json")
...
let prekey = []
let directorio = readdirSync("./SharksBotSession")
let filesFolderPreKeys = directorio.filter(file => {
return file.startsWith('pre-key-')
})
prekey = [...prekey, ...filesFolderPreKeys]
filesFolderPreKeys.forEach(files => {
unlinkSync(`./SharksBotSession/${files}`)
})
} 

function purgeSessionSB() {
try {
const listaDirectorios = readdirSync('./SharksBotMd/');
let SBprekey = [];
listaDirectorios.forEach(directorio => {
if (statSync(`./SharksBotMd/${directorio}`).isDirectory()) {
const DSBPreKeys = readdirSync(`./SharksBotMd/${directorio}`).filter(fileInDir => {
return fileInDir.startsWith('pre-key-')
})
SBprekey = [...SBprekey, ...DSBPreKeys];
DSBPreKeys.forEach(fileInDir => {
if (fileInDir !== 'creds.json') {
unlinkSync(`./SharksBotMd/${directorio}/${fileInDir}`)
}})
}})
...
const directories = ['./SharksBotSession/', './SharksBotMd/']
