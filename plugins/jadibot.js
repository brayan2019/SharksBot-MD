...
const isCommand3 = /^(listsharkbot|bots|subsbots|subbots)$/i.test(command)  
...
const path = `./SharksBotMd/${uniqid}`
let comd = `${lenguajeGB.lenguaje() == 'es' ? 'serbot' : 'sharkbot'}`
...
try {
fs.rmdir("./SharksBotMd/" + uniqid, { recursive: true, force: true })
...
case isCommand3:
//if (global.db.data.settings[conn.user.jid].sharkbotmd) return m.reply(`${lenguajeGB['smsSoloOwnerJB']()}`)
...
handler.command = /^(deletesesion|eliminarsesion|borrarsesion|delsesion|delsession|cerrarsesion|berhenti|pausesb|detenersb|pausarsb|listsharkbot|bots|subsbots|subbots)$/i
