...
case 'sharkbotmd': case 'modesharkbot': case 'serbotmd': case 'modoserbot': 
isAll = true
if (!isROwner) {
global.dfail('rowner', m, conn)
throw false
}
bot.sharkbotmd = !isEnable
break 
...
${lenguajeGB.smsParaOw() + ' ' + `${bot.sharkbotmd ? '✅' : '❌'}`}
🦈 \`\`\`${usedPrefix}on/off\`\`\` *${lenguajeGB.lenguaje() == 'es' ? 'modoserbot' : 'modesharkbot'}*\n
