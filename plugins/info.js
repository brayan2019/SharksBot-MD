...
const isCommand3 = /^(gata(bot)?groups?|grupos(ofc|deSharkBot|gb)|grupogb|groupsgb|gatagroups?|grupos|group(ofc|sofc))$/i.test(command) 
...
*╰𝄗𝄗𝄗𝄗𝄗⬣*`.trim()
await conn.sendFile(m.chat, gataImg.getRandom(), 'shark.jpg', estado, fkontak)
} catch (e) {
reportError(e)
}   
break
...
_⭔ ${grupo[9]}_\n
_⭔ ${grupo[10]}_`.trim()
await conn.sendFile(m.chat, gataImg.getRandom(), 'shark.jpg', str, fkontak)   
} catch (e) {
reportError(e)
}    
break
...
await conn.sendFile(m.chat, gataImg.getRandom(), 'shark.jpg', `*_${_package.homepage}_*\n\n` + lenguajeGB.smsPrivadoDonar(), fkontak)    
} catch (e) {
reportError(e)
}    
break
...
let replit = `*◎ R E P L I T*\n\nhttps://replit.com/github/brayan2019/SharksBot-MD`
let heroku = `*◎ H E R O K U*\n\nhttps://heroku.com/deploy?template=https://github.com/brayan2019/SharkBotMDLite-Heroku`
...
totalreg = Object.keys(global.db.data.users).length
rtotalreg = Object.values(global.db.data.users).filter(user => user.registered == true).length
let totalsharkbot = [...new Set([...global.conns.filter((conn) => conn.user && conn.ws.socket && conn.ws.socket.readyState !== ws.CLOSED).map((conn) => conn)])];
...
🌼꙰᠁❥ *◜${lenguajeGB.smsBT9()}◞* ⇢ ${totalsharkbot.length}`.trim()

await conn.sendFile(m.chat, gataImg.getRandom(), 'shark.jpg', info, fkontak) 
} catch (e) {
reportError(e)
}     
break

//CÓDIGO CREADO GRACIAS A https://github.com/Azami19 & https://github.com/brayan2019
case isCommand9:
try{
...
description = official[0][0] == String(contact[0]) ? 'Solo temas de SharkBot' : official[1][0] == String(contact[0]) ? lenguajeGB.smsContacto4() : official[2][0] == String(contact[0]) ? lenguajeGB.smsContacto4() : official[3][0] == String(contact[0]) ? lenguajeGB.smsContacto4() : desc === '' ? lenguajeGB.smsContacto5() : desc
correo = official[0][0] == String(contact[0]) ? 'xlbrayan30@gmail.com' : official[1][0] == String(contact[0]) ? 'xlbrayan30@gmail.com' : official[2][0] == String(contact[0]) ? 'xlbrayan30@gmail.com' : mail === '' ? lenguajeGB.smsContacto6() : mail
lugar = official[0][0] == String(contact[0]) ? '🇩🇴 DOMINICANO' : official[1][0] == String(contact[0]) ? '🇩🇴 DOMINICANO' : official[2][0] == String(contact[0]) ? '🇩🇴 DOMINICANO' : official[3][0] == String(contact[0]) ? '🇩🇴 DOMINICANO' : country === '' ? lenguajeGB.smsContacto7() : country
enlace = official[0][0] == String(contact[0]) ? 'https://www.instagram.com/soybryant_hdx' : official[1][0] == String(contact[0]) ? 'https://www.instagram.com/soybryant_hdx' : official[2][0] == String(contact[0]) ? 'https://www.instagram.com/soybryant_hdx' : official[3][0] == String(contact[0]) ? 'https://www.instagram.com/soybryant_hdx' : md    
lista.push([number, ofc, nombre, description, official[3][0] == String(contact[0]) ? null : correo, lugar, enlace, bio, official[1][0] == String(contact[0]) ? 'https://www.instagram.com/soybryant_hdx' : null]) }  
lista.push([conn.user.jid.split('@')[0], await conn.getName(conn.user.jid), packname, lenguajeGB.smsContacto8(), mail === '' ? 'xlbrayan30@gmail.com' : mail, lenguajeGB.smsContacto7(), md, bioBot, yt, ig, fb, paypal, nna])
...
await conn.sendFile(m.chat, gataImg.getRandom(), 'shark.jpg', caption.trim(), fkontak) 
} catch (e) {
reportError(e)
}   
break
...
await conn.sendFile(m.chat, gataImg.getRandom(), 'shark.jpg', lenguajeGB.smsTituloDonar() + '\n\n' + donar + '\n\n' + paypal + '\n' + ig, fkontak) 
