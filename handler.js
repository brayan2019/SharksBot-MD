...
if (!('sharkbotmd' in settings)) settings.sharkbotmd = true  
...
this.reply(m.chat, `🦈 *NO TIENE SHARKSCOINS*`, m)
...
if (m.money)
m.reply(+m.money + ' *SHARKSCOINS USADO(S)* 🦈')
break
}}} catch (e) {
console.error(e)
} finally {
...
text = (action === 'add' ? (chat.sWelcome || this.welcome || conn.welcome || 'Welcome, @user!').replace('@subject', await this.getName(id)).replace('@desc', groupMetadata.desc?.toString() || '🦈 Sharks Bot MD 🦈') :
...
let prep = generateWAMessageFromContent(m.chat, { extendedTextMessage: { text: msg, contextInfo: { externalAdReply: { title: lenguajeGB.smsAvisoAG().slice(0,-2), body: [wm, '🦈 Sharks ' + gt + ' 🦈', '🦈 xlbrayan30.gmail.com'].getRandom(), thumbnail: gataImg.getRandom(), sourceUrl: [md, yt, nna, nn, nnn, ig, paypal, fb].getRandom() }}}}, tg)
